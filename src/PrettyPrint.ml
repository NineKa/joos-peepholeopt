...
let rec prettyprint_expression = fun (t :AST.expression) ->
  ...
  | AST.InvokeExpr({target = target; operands = operands}) ->
     let buffer = Buffer.create 32 in
...
let prettyprint_statement = fun (statement :AST.statement) ->
  ...
  | AST.LiteralLabel {name = name}        -> Printf.sprintf "%s :" name
  | AST.CaptureLabel {name = name}        -> Printf.sprintf "[%s] :" name
  | AST.BlockOneStmt {name = name}        -> Printf.sprintf "< [%s] >" name
  | AST.BlockOneOrMoreStmt {name = name}  -> Printf.sprintf "< [%s]+ >" name
  | AST.BlockZeroOrMoreStmt {name = name} -> Printf.sprintf "< [%s]* >" name
...
let prettyprint_pattern = fun (pattern :AST.pattern) ->
  ...
  List.iter
    (fun (precondition :AST.expression) ->
      let single_line =
        Printf.sprintf "  .pre %s\n" (prettyprint_expression precondition)
      in Buffer.add_string buffer single_line)
